...
class SQLExample(object):
            ...
            if conditions is not None and len(conditions) > 0:
                ...
                for cond in conditions:
                    ...
                    for i in range(len(self.tokens)):
                        ...
                        e = len(question) if i + val_len >= len(self.word_to_char_start) else self.word_to_char_start[
                            i + val_len]
                        recovered_answer_text = question[s:e].strip()
                        answer_text_minjie = ''
                        for idx in range(i,i+val_len):
                            answer_text_minjie += self.tokens[idx]
                        value_minjie = ''
                        for idx in range(len(value)):
                            if value[idx] == ' ':
                                continue
                            else:
                                value_minjie += value[idx]
                        #edit by Minjie
                        if value.lower() == recovered_answer_text.lower():
                            cur_start = i
                            break
                        elif value_minjie.lower() == answer_text_minjie.lower():
                            cur_start = i
                            question_before = question
                            question = question[:s]+value.lower()+question[e:]
                            self.tokens, self.char_to_word, self.word_to_char_start = basic_tokenize(question)
                            #print(value,question,question_before)
                            break
                        else:
                            continue
    ...
    def load_from_json(s):
        d = json.loads(s)
        keys = ["qid", "question", "table_id", "column_meta", "agg", "select", "conditions", "tokens", "char_to_word",
                "word_to_char_start", "value_start_end", "valid"]
